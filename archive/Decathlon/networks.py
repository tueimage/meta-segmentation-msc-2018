...
from utils import generator, historyPlot, create_data_subsets, dice_coef_loss, auc, mean_iou, dice_coef
import os
...
class EncoderDecoderNetwork():
    ...
    def load_weights(self):
        self.weights_file = "/home/tjvsonsbeek/featureExtractorUnet/models/final_model_{}_{}".format(self.task, self.name)
    ...
    def update_encoder_weights(self):
        if self.name == 'VGG16':
            ...
            self.feature_extractor = Model(input = self.model.input, output = self.model.layers[173].output)
    ...
    def train(self, train_data, val_data, imageDimensions, verbosity):
        self.history = self.model.fit_generator(generator(train_data, self.minibatch_size, imageDimensions), steps_per_epoch = int(np.ceil(len(train_data)/self.minibatch_size)), nb_epoch = self.epochs, validation_data = generator(val_data, self.minibatch_size, imageDimensions), validation_steps = int(np.ceil(len(val_data)/self.minibatch_size)), verbose = verbosity)
    def save_model(self):
        print("---SAVING MODEL---")
        self.model.save_weights("models/final_model_{}_{}".format(self.task, self.name))
    ...
    def build_decoder_RESNET50(self):
        conv5 = Conv2D(2048, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(self.feature_extractor.layers[173].output)
        up6 = Conv2D(2048, 2, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(UpSampling2D(size = (2,2))(conv5))
        merge6 = concatenate([self.feature_extractor.layers[141].output,up6], axis = 3)
        ...
        merge7 = concatenate([self.feature_extractor.layers[79].output,up7], axis = 3)
        conv7 = Conv2D(512, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(merge7)
        conv7 = Conv2D(512, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv7)
        ...
        merge9 = concatenate([self.feature_extractor.layers[4].output,up9], axis = 3)
        conv9 = Conv2D(128, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(merge9)
        conv9 = Conv2D(128, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv9)
    ...
    def build_decoder_MobileNetV1(self):
        up6 = Conv2D(1024, 2, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(UpSampling2D(size = (2,2))(self.feature_extractor.layers[86].output))
        merge6 = concatenate([self.feature_extractor.layers[73].output,up6], axis = 3)
        conv6 = Conv2D(1024, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(merge6)
        conv6 = Conv2D(1024, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv6)

        up7 = Conv2D(1024, 2, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(UpSampling2D(size = (2,2))(conv6))
        merge7 = concatenate([self.feature_extractor.layers[36].output,up7], axis = 3)
        conv7 = Conv2D(512, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(merge7)
        conv7 = Conv2D(512, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv7)

        up8 = Conv2D(512, 2, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(UpSampling2D(size = (2,2))(conv7))
        merge8 = concatenate([self.feature_extractor.layers[23].output,up8], axis = 3)
        conv8 = Conv2D(256, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(merge8)
        conv8 = Conv2D(256, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv8)

        up9 = Conv2D(256, 2, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(UpSampling2D(size = (2,2))(conv8))
        merge9 = concatenate([self.feature_extractor.layers[10].output,up9], axis = 3)
        conv9 = Conv2D(128, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(merge9)
        conv9 = Conv2D(128, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv9)
